...
class AgenteL:  
    def __init__(self, matriz, PI, PF):
        ...
        self.PI = PI
        self.PF = PF
        self.mascara = crearMascara(len(matriz), len(matriz))
     
    #Funcion para manejar el agente para laberintos de manera manual   
    def Manual(self):
        PA = PI
        PDec = []
        PVis = []
        libres = sensor4dir(matriz, self.mascara, PA[0], PA[1])
        if libres > 1:
                PDec.append(PA)
        ...
        dibujarCuadricula(self.mascara, "L")
        marcarMapa("INICIO",PI[0], PI[1])
        marcarMapa("ACTUAL", PA[0], PA[1])
        marcarPuntos(PVis, "VISITADO")
        marcarPuntos(PDec, "DECISION")
        marcarMapa("FINAL", self.PF[0], self.PF[1])
        pygame.display.update() 
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if ESTADO == "DIBUJAR MASCARA":
            A1 = AgenteL(matriz, PI, PF).Manual()